...
function legalMove(st, rng) {
  ...
  switch (st.phase) {
    ...
    case "propose": {
      const pool = [...Array(st.n).keys()].filter((s) => s !== st.wounded && !(st.hour === "recused" && s === st.leader));
...
test("Recused: the leader cannot join their own team; the card never comes on the last two missions, nor when no clean team is possible", () => {
  ...
  assert.throws(() => propose(st, [lead, others[0]]), /recused/);
  st = propose(st, others.slice(0, 2));
  assert.equal(st.phase, "vote");
